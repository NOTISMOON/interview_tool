...
>
> **状态（2026-09-06 复核）**：本文档为立项期（2026-08-18）的规划清单。下列模块均已实现，实现细节以 `docs/功能模块流程/`、`docs/DB设计/数据库设计文档.md` 与代码为准：
> - 用户管理 / 简历管理（上传+去重+AI 解析+删除/重试）✅
> - 面试管理（LangGraph 出题/Fast Decision/异步判题与报告/设备检测/软删除）✅
> - 社区帖子/评论（含评论点赞 comment_like）/点赞/收藏/标签 ✅
> - 社交关注（Outbox 写路径 + ZSET/SET 读路径 + 关注动态通知 type=7）✅
> - 消息通知（SSE 增量拉取 + type 1-7 + unread 合并私信）✅
> - 私信聊天（Redis Stream 写缓冲 + WS + 幂等落库 + 扇出）✅
> - 文件上传（COS 前端直传 + 回调校验 + upload_records）✅
> - AI 服务集成（LangGraph 工作流：resume_parse / interview）✅
> - 打卡签到（checkin，Redis Bitmap）✅
> - 单设备登录（jti 顶号 + session_kicked）✅
>
> 本文档版本：v1.0 | 日期：2026-08-18 | 状态：**规划存档（已基本全部实现）**
